...
- [x] **Two-file package split** still config-checks clean on 2024.10 + 2025.4 (already true; re-confirm after edits). Re-confirmed 2026-07-26 after all Phase 3 test-env edits (`virtual.yaml`, `configuration.yaml`, `automations.yaml`).
- [x] **Mid-move manual stop** → layer-3 suspend (Phase 1 Step 7, run as a standing regression on a watched virtual cover). PASS, `.superpowers/sdd/task-3-report.md` (R1).
- [x] **Wall-switch move** (contextless, no live marker) → `manual_check` layer 2 suspends. Simulate: change a virtual cover's state with no `moving_*` marker present and no `user_id`; expect suspension. The original R2 run (task-3-report.md) actually exercised layer 1 (a REST call always carries `user_id`) — redone correctly 2026-07-26 using HA's context-recency rule (`CONTEXT_RECENT_TIME_SECONDS`, 5s) to get a genuinely contextless tick with no marker present. PASS — see `docs/test-environment.md` gotchas and `test-r2-layer2-contextless.ps1`.
- [x] **Physical weather-blocked close, cleanly staged** (the one weather case left unproven): configure a `time`-method instance that genuinely _wants closed_ (close_time in the past, open_time in the future) so the heartbeat agrees, `weather_entity: weather.weather_at_8001`. With `weather_events:[sunny]` (current) the cover must **stay open**; flip to `[rainy]` and it must **close**. Avoids the daytime sun-heartbeat fight documented in memory. PASS 2026-07-26 (used `[partlycloudy]`/`[rainy]`, matching the live condition that day) — repointed `cover_time_test` at `cover.vfast`; see `docs/test-environment.md`.
- [x] **Restart mid-transition self-heal**: set `_commanded` stale vs desired, restart HA, confirm catch-up on the `homeassistant: start` heartbeat; and first-run `-1` sentinel drives covers to phase. PASS 2026-07-26 — `cover_time_test`/`cover.vfast` for the stale-but-present case, `cover_brightness_test`/`cover.vbright` for the `-1`-sentinel + persisted-stabilizer-timestamp case (`test-restart-selfheal.ps1`). Note the "`_commanded`" key named here is Phase-1 vocabulary; Phase 2 replaced it with the instance's own published intent (`shared.<id>::intent`) plus `<cover>::resolved` — both must be set for a valid stale/never-run precondition, not just the intent (an earlier attempt at this test forgot to also clear `<cover>::resolved` and got a false negative — see the script's own note).
- [x] **Brightness stabilizer edges**: `<id>_lux_above_since` / `_below_since` written only on threshold crossings; `schedule_wants_*` respects `stabilizer_seconds`. PASS 2026-07-26 — added `input_number.test_lux`/`sensor.test_lux` and a dedicated `cover.vbright`, repointed `cover_brightness_test`; see `test-brightness-stabilizer.ps1`.
- [ ] Native-bool parsing of `schedule_wants_open/closed` / `stabilizer_needs_update` in raw `{% if %}` (the `result_as_boolean` gotcha) — confirm they behave as real booleans at runtime. Not independently isolated, but the weather, brightness and self-heal checks above all depend on these same variables rendering as correct booleans across many runs and all behaved correctly — left unchecked because that's incidental coverage, not a dedicated test.
