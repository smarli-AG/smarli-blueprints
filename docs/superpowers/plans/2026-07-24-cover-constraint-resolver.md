...
Three inaccuracies found while executing this plan (task reports and the Task 6 re-sweep), recorded here rather than rewritten into the task bodies above:

- **Task 4, Step 8 (A10) as written does not test what it claims to.** Arming `stub.day` to hand the resolver a competing `min: 50` bound makes the `stub_day` automation itself fire and overwrite `shared.stub_day::intent` with its own hard-coded `bounds: {}` _before_ the resolver reads it — the conflict is gone by the time resolution happens, and the observed answer (20) comes from the ordinary `max: 20` bound alone, the same as A1, not from the infeasible-bounds-drop-the-loser logic. Drive the conflict through `stub_shade` instead — or, as verified in the Task 6 sweep, hand-write **both** competing `shared.*::intent` keys directly via `smarli_tracker_set` (bypassing both stub automations' own arm/overwrite cycle) and trigger a resolve pass with a bare `script.turn_on` on `script.smarli_cover_resolve` (no `owner`/`intent` data) so neither stub automation gets a chance to fire and self-overwrite its hand-written intent first.
- **Task 5, Step 1's test uses the wrong entity_id.** `automation.cover_sun_test` does not exist. HA derives an automation's entity_id from its **alias** ("Covers Day-Night — Sun"), not its config `id:` field — the real entity_id is `automation.covers_day_night_sun`, whose `attributes.id` is still `cover_sun_test`. Use the real entity_id to look up the automation state; `attributes.id` off of it is what the rest of the test needs.
- **The orientation block's blueprint deploy path has a doubled segment.** `C:\Users\Pascal\smarli-ha-test\config\config\blueprints\automation\smarli\coversDayNight.yaml` does not exist. The real path is `C:\Users\Pascal\smarli-ha-test\config\blueprints\automation\smarli\coversDayNight.yaml` (one `config`, not two).
