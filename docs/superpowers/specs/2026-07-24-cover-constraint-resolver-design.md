...
- **Settle-audit suspension has a narrow compound-failure window.** The layer-3 audit calls `script.smarli_cover_suspend` with `fallback_duration: 0`, relying on the commanding automation's intent still being active moments later so its `suspension_duration` is what actually gets applied. If, between the command and the audit, the commanding automation is disabled or its intent's optional `until` lapses, no active intent names the cover anymore, `smarli_cover_suspend`'s duration resolves to 0, and a genuine mid-move manual stop is silently **not** suspended. Window: command → audit, bounded by `settle_timeout`. Loss: one missed suspension in that narrow, already-unlikely case (a technician disabling the very automation mid-move, or an `until` timed to expire in that exact multi-second window). A hardcoded fallback was tried and removed on purpose (§6, "Suspension duration is resolved centrally"): it would suspend covers whose contributors deliberately opted out of suspension (`suspension_duration: 0`), which is worse than the rare miss it would fix. Accepted as-is — and note the miss is close to inert: the duration only collapses to 0 when **no** live contributor names the cover, which is exactly the situation where no automation is left to re-drive it. A suspension exists to hold automations off a cover; with every contributor for that cover disabled there is nothing to hold off. If a second contributor is still on, its intent supplies the duration and the suspension is written normally.
- **A stale `targets` entry can outlive its cover selection.** A day-night instance republishes its intent only when `transition_needed` flips (§5, contributor contract). If a technician removes a cover from the instance's `cover_entity` input, the previously published `targets` dict keeps naming that cover at the old target until the next genuine phase transition republishes the corrected set. Self-healing (the next transition fixes it) and low-impact (the resolver only commands on a value change, and the cover already sits at the stale target in the common case), so accepted rather than triggering a republish on every input change.
