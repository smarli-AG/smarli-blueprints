...
| V6  | `script.smarli_cover_move`'s `current` attribute reflects **queued** runs                                                                                                                                                 | ✅ `script.smarli_cover_move`'s state object exposes a `current` attribute (observed value `0` at rest, alongside `max`), confirming the key exists and is readable. Coalescing logic that reads `current` remains viable as designed.                                                                                  |

Test-env scaffolding used for this pass (temporary `v3_probe` automation, temporary `recorder: exclude` block, temporary `v2`/`v3`/`v4` tracker namespaces) was removed/purged afterward; the container was restored to its prior image (`2025.4`) and confirmed back on `2025.4.4`.
