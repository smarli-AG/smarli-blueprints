...
3. Per cover **C**:
   - **Target** = the active intent with the newest `ts` that names C in `targets`. Tie → higher `priority`; still tied → lexicographically by owner id, so the result is deterministic rather than dict-order luck. If no intent names C in `targets`: use `<C>::resolved`; if that is also absent (the very first resolve for this cover), use its current position — and if the cover is `unavailable`/`unknown` so no position can be read, skip C this run rather than guessing.
...
If another resolve run is already queued behind this one, **return immediately without writing or commanding**. The queued run has strictly fresher input and will compute the final answer. This removes the redundant intermediate command in the coincident case — and with it the visible "cover starts toward 100, reverses to 20" twitch. Depends on `script.smarli_cover_resolve`'s `current` attribute reflecting queued runs (verification V6); if it does not, the only loss is cosmetic.
...
| File                             | Change                                                                                                                                                                                                 |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `packages/smarli_core.yaml`      | + `script.smarli_tracker_gc`, + `automation.smarli_tracker_gc`; tracker semantics unchanged                                                                                                            |
...
|     | Check                                                                                                                                                                                                                     | If it fails                                                                                                                                                                                       |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| V1  | Automation state objects expose `attributes.id`, on **both** 2024.10 and 2025.4                                                                                                                                           | Owner-liveness and GC need another handle → fall back to `until` stamps. Also report the HA version that introduced the attribute, so raising `min_version` is a decision with numbers behind it. |
| V2  | A trigger-based template sensor processes its triggers strictly in event order under burst load                                                                                                                           | Contributors pass their intent _through_ the resolve call and the resolver performs the tracker write — one writer, no lag                                                                        |
| V3  | `automation_reloaded` fires after the reloaded entities exist                                                                                                                                                             | Lengthen the GC settling delay, or trigger GC differently                                                                                                                                         |
| V4  | Excluding `sensor.smarli_automation_tracker` from the recorder does not break its restore across restarts (trigger-based template sensors are expected to restore via `.storage/core.restore_state`, not the recorder DB) | Keep it recorded; rely on GC alone to bound growth                                                                                                                                                |
| V5  | Behaviour at HA's state-attribute size limit (informational — quantifies the growth risk)                                                                                                                                 | —                                                                                                                                                                                                 |
| V6  | `script.smarli_cover_resolve`'s `current` attribute reflects **queued** runs                                                                                                                                              | Drop coalescing; the loss is cosmetic (the twitch returns)                                                                                                                                        |
...
| A4  | Both contributors publish conflicting intents in the same tick, repeated | Correct final position, **no false suspension**; one command if V6 holds, otherwise two and a visible twitch — never a suspension                                                                                                                    |
